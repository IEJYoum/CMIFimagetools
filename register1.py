...
def read(folder,keyChannel=1,keyRound=1,scene=1):
...
def cost_function_registration(folder):
    fileNames = read(folder)  #rename read and main (to be more unique) 
    try:
        key = load(folder,fileNames[0,0]) 
    except:
    ...
    for i in range(1,fileNames.shape[0]):
        ...
        file.save(folder+"registered-"+name+'.tif')
        file.close  
        ...
        for j in range(1,fileNames.shape[1]):
            if fileNames[i,j] != 0:
                try:
                    ...
                    followingA = load(folder,followName)
                    for k in range(len(paths)):
                        followingA = followPath(followingA,paths[k],levels[k])
                    file = Image.fromarray(followingA)
                    file.save(folder+"registered-"+followName+'.tif')
                    file.close  
                except:
                        print("error processing",fileNames[i,j])

if __name__ == '__main__':
    cost_function_registration(folder="C:/Users/youm/Desktop/Registration/210405/")
